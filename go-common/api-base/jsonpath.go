...
type JsonValueNotFound struct {
	content string
}

func (e *JsonValueNotFound) Error() string {
	return e.content
}

func makeJsonValueNotFoundError(format string, args ...interface{}) *JsonValueNotFound {
	return &JsonValueNotFound{
		content: fmt.Sprintf(format, args...),
	}
}

func IsJsonValueNotFound(err error) bool {
	_, ok := err.(*JsonValueNotFound)
	return ok
}

func JsonPathLookup(root interface{}, jpath string) (interface{}, error) {
	...
	for _, s := range steps {
		...
		case "filter":
			...
			xobj, err = get_filtered(xobj, root, args.(string))
		default:
...
func get_key(obj interface{}, key string) (interface{}, error) {
	...
	case reflect.Map:
		...
		return nil, makeJsonValueNotFoundError("key error: %s not found in object", key)
	case reflect.Slice:
		...
		return res, nil
	default:
		return nil, makeJsonValueNotFoundError("object is not map")
	}
}
...
func get_idx(obj interface{}, idx int) (interface{}, error) {
	...
	case reflect.Slice:
		...
		if idx >= 0 {
			if idx >= length {
				return nil, makeJsonValueNotFoundError("index out of range: len: %v, idx: %v", length, idx)
			}
	...
	default:
		return nil, makeJsonValueNotFoundError("object is not Slice")
	}
}
...
func get_range(obj, frm, to interface{}) (interface{}, error) {
	...
	case reflect.Slice:
		...
		if _frm < 0 || _frm >= length {
			return nil, makeJsonValueNotFoundError("index [from] out of range: len: %v, from: %v", length, frm)
		}
		if _to < 0 || _to > length {
			return nil, makeJsonValueNotFoundError("index [to] out of range: len: %v, to: %v", length, to)
		}
